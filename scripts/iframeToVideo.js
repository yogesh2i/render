...
/**
 * SDK function - orchestrates the conversion process
 */
async function convertUrlsToVideos({ urls, duration, outputDir, inputFile }) {
	...
	try {
		...
		const formattedUrls = urls.map(url => ({ url, duration }));
...
module.exports = { convertUrlsToVideos };
