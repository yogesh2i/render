...
export const POST = async (request: NextRequest) => {
  try {
    ...
    // Use /tmp for output on Vercel
    const bundleInfoPath = path.join(process.cwd(), 'bundle-info.json');
    if (!fs.existsSync(bundleInfoPath)) {
      throw new Error('Bundle not found. Please run "npm run bundle" first.');
    }

    const bundleInfo = JSON.parse(await fs.promises.readFile(bundleInfoPath, 'utf8'));
    const bundleLocation = bundleInfo.bundleLocation;
    ...
    const timestamp = Date.now();
    const filename = `overlay-${timestamp}.mp4`;
    const outputLocation = path.join('/tmp', filename); // Vercel-compatible
    ...
    // If you want to serve the file, you may need to copy it to /public or stream it directly
